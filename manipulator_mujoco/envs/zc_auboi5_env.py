...
from manipulator_mujoco.robots import Arm, ENDLINK,AG95
...
class ZcAuboI5Env(gym.Env):
    ...
    def __init__(self, render_mode=None):
        ...
        # aubo i5 arm
        self._arm = Arm(xml_path=os.path.join(
        os.path.dirname(__file__),
        '../assets/robots/Zc_aubo_i5/aubo_i5.xml',
        ),
        ...
        self._box = Primitive(type="box", size=[0.02,0.02,0.02], pos=[0,0,0.05], rgba=[1, 0, 0, 1], friction=[1, 0.3, 0.0001],mass=0.1)

        #plate to be manipulated
        ...
        self._arena.attach_free(
            self._box.mjcf_model, pos=[0.5,0,0.25]
        )
    ...
    def step(self, action: np.ndarray) -> tuple:
        ...
        print("o_target_pose",target_pose[:3])
        
        # get mocap target pose
        for i in range(0,3):
            target_pose[i] = target_pose[i]+action[i]
            if target_pose[i]>0.8865 :
                target_pose[i] = 0.8865
            elif target_pose[2]< 0 :
                target_pose[2] = 0
        print("target_pose",target_pose[:3])

        self._target.set_mocap_pose(self._physics, target_pose[:3], target_pose[3:])
