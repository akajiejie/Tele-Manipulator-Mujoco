...
def draloha_fk(theta1, theta2, theta3, theta4, theta5, theta6):
    ...
    return T06
#dr arm initalization

kp=886.5/476.81#单位为mm
ko=0.8
dt=0.016 #更新频率60Hz
...
def Rotate2Quaternionf(euler):
    r = R.from_euler('ZYZ', euler, degrees=True)#生成旋转矩阵
    quaternion = r.as_quat()#旋转矩阵转四元数
    return quaternion
...
def main():
    ...
    end_post=np.empty((2,3))
    #dr arm init
    # for j in range(len(dr.ID_list)):#停止1-6号关节电机工作
    #     dr.estop(dr.ID_list[j])
    
    # Create the environment with rendering in human mode
    env = gymnasium.make('manipulator_mujoco/ZcAuboI5Env-v0', render_mode='human')
    ...
    while True:
        ...
        if ((time.time()-start_time)>=dt):
            ...
            Gp_angle=dr.get_angle(7)#获取夹爪电机位置
            for i in range(0,3):
                target_pose[i]=((end_post[0][i]-old_post[0][i])/1000*kp)#计算位移增量
            # for i in range(0,4):
            #     target_quaternion[i]=((end_quaternion[i]-old_quaternion[i]))#计算旋转增量（暂时不传递旋转）
            
            print("d_target_post",target_pose)
            # print("post euler",quaternion2euler(end_quaternion))
            d_action= np.concatenate((target_pose, target_quaternion), axis=0)#合并位姿增量
            start_time=time.time()
            old_post=end_post
            old_quaternion=end_quaternion#保存上一次sample的位姿

        # Take a step in the environment using the chosen action
        observation, reward, terminated, truncated, info = env.step(d_action)
